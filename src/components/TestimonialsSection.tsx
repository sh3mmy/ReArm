      ...
      <div className="relative max-w-7xl mx-auto px-6 lg:px-8">
        <div className={`mb-20 transition-all duration-800 ${isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <h2 className="font-normal text-white mb-6 tracking-[-0.02em] leading-[1.05]" style={{ fontSize: 'clamp(1.75rem, 3vw, 2.5rem)' }}>
            What Experts and Users<br /><span className="text-neutral-400">Are Saying.</span>
          </h2>
        </div>

        <div className="grid lg:grid-cols-3 gap-6 lg:gap-8">
          {testimonials.map(({ quote, author, title, org, delay }, index) => (
            <div key={index} className={`group card-premium p-8 lg:p-10 flex flex-col transition-all duration-800 ${isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'}`} style={{ transitionDelay: `${delay}ms` }}>
              <div className="mb-6"><Quote size={28} className="text-accent-400/40" strokeWidth={1} /></div>
              <blockquote className="font-light text-neutral-300 leading-relaxed flex-1 mb-8 text-base">"{quote}"</blockquote>
              ...
              <div className="absolute bottom-0 left-0 right-0 h-px bg-gradient-to-r from-accent-400/50 via-accent-400/20 to-transparent scale-x-0 group-hover:scale-x-100 transition-transform duration-700 origin-left" />
            </div>
          ))}
        </div>
      </div>
