  ...
  const footerLinks = [
    ...
    },
    {
      ...
      links: [
        ...
        { label: 'Configurator', href: '/your-arm' },
      ]
    },
    {
      title: 'Company',
      links: [
        { label: 'Philosophy', href: '/the-future' },
        { label: 'Vision', href: '/the-future' },
        { label: 'Careers', href: '#' },
      ]
    }
  ];
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8 py-20 lg:py-24">
        <div className="grid lg:grid-cols-12 gap-12 lg:gap-8">
          ...
          <div className="lg:col-span-4">
            ...
            <p className="body-premium text-neutral-500 text-sm max-w-xs leading-relaxed">
            ...
            <div className="flex items-center gap-4 mt-8">
              {socialLinks.map(({ icon: Icon, href, label }) => (
                <a
                  key={label}
                  href={href}
                  className="w-10 h-10 rounded-full border border-white/[0.08] flex items-center justify-center text-neutral-500 hover:text-white hover:border-white/20 transition-all duration-300"
                  aria-label={label}
                >
            ...
            <div className="grid grid-cols-3 gap-8">
              {footerLinks.map((section) => (
                <div key={section.title}>
                  <h3 className="text-label text-neutral-500 mb-4">{section.title}</h3>
                  <ul className="space-y-3">
                    {section.links.map((link) => (
                      <li key={link.label}>
                        <Link
                          to={link.href}
                          className="text-sm text-neutral-400 hover:text-white transition-colors duration-300 link-underline inline-block"
                        >
                          {link.label}
                        </Link>
                      </li>
        ...
        <div className="mt-20 pt-8 border-t border-white/[0.06] flex flex-col md:flex-row justify-between items-center gap-4">
          <p className="text-neutral-600 text-xs tracking-wide">
            © 2025 ReArm®. All rights reserved.
          </p>
