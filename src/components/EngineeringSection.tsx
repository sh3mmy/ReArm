    ...
    <section id="engineering" className="relative py-32 lg:py-40 bg-neutral-950 overflow-hidden" ref={ref}>
      {bgSrc && (
        <div className="absolute inset-0">
          <img src={bgSrc} alt="" aria-hidden className="absolute inset-0 w-full h-full object-cover select-none pointer-events-none opacity-15" />
          <div className="absolute inset-0 bg-gradient-to-b from-neutral-950 via-neutral-950/80 to-neutral-950" />
        </div>
      )}

      <div className="relative max-w-4xl mx-auto px-6 lg:px-8">
        <div className={`mb-20 transition-all duration-800 ${isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <h2 className="font-normal text-white mb-6 tracking-[-0.02em] leading-[1.05]" style={{ fontSize: 'clamp(1.75rem, 3vw, 2.5rem)' }}>
            Where Craftsmanship<br /><span className="text-neutral-400">Meets Innovation.</span>
          </h2>
        </div>

        {sections.map((section, index) => (
          <div key={index} className={`mb-28 lg:mb-36 last:mb-0 transition-all duration-800 ${isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`} style={{ transitionDelay: `${index * 200}ms` }}>
            <div className="flex items-center gap-4 mb-6">
              <span className="section-label text-accent-400/70">{section.label}</span>
              <div className="flex-1 h-px bg-gradient-to-r from-white/10 to-transparent" />
            </div>
            <h3 className="font-normal text-3xl lg:text-5xl text-white mb-8 tracking-tight leading-[1.05]">{section.title}</h3>
            <div className="space-y-5 max-w-3xl">
              {section.content.map((paragraph, pIndex) => (
                <p key={pIndex} className="font-light text-lg lg:text-xl text-neutral-400 leading-relaxed">{paragraph}</p>
              ))}
            </div>
            {index < sections.length - 1 && (
              <div className="mt-16 flex items-center gap-4">
                <div className="divider-premium" />
                <span className="section-label text-neutral-600">{String(index + 2).padStart(2, '0')}</span>
              </div>
            )}
          </div>
        ))}
