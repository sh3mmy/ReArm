...
const PossibilitiesSection: React.FC = () => {
  ...
  const possibilities = [
    { number: '01', title: 'Precision Control', description: 'Sub-millimeter accuracy in every movement, powered by neural processing.' },
    { number: '02', title: 'Adaptive Learning', description: 'AI that evolves with you, learning your unique patterns and preferences.' },
    { number: '03', title: 'Limitless Expression', description: 'From delicate artistry to extreme athletics — no boundaries, only possibilities.' },
  ];
      ...
      <div className="relative max-w-7xl mx-auto px-6 lg:px-8">
        <div className={`max-w-3xl mb-20 transition-all duration-800 ${isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <h2 className="font-normal text-white mb-6 tracking-[-0.02em] leading-[1.05]" style={{ fontSize: 'clamp(1.75rem, 3vw, 2.5rem)' }}>
            Redefining<br /><span className="text-neutral-400">Possibilities.</span>
          </h2>
        ...
        <div className="grid lg:grid-cols-3 gap-6 lg:gap-8">
          {possibilities.map((item, index) => (
            <div key={index} className={`group card-premium p-8 lg:p-10 transition-all duration-800 ${isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'}`} style={{ transitionDelay: `${index * 150}ms` }}>
              <div className="flex items-center justify-between mb-8">
                <span className="section-label text-accent-400/60">{item.number}</span>
              ...
              <h3 className="text-xl lg:text-2xl font-medium text-white mb-4 tracking-tight">{item.title}</h3>
              <p className="font-light text-neutral-500 leading-relaxed">{item.description}</p>
              <div className="mt-8 h-px bg-gradient-to-r from-accent-400/40 via-accent-400/20 to-transparent scale-x-0 group-hover:scale-x-100 transition-transform duration-600 origin-left" />
            </div>
          ))}
        </div>
      </div>
